import Ctabutton from "./atoms/ctabutton";

export default function Steps() {
  return (
    <div className="px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20">
      <div className="max-w-xl mb-10 mx-auto text-center lg:max-w-2xl md:mb-12">
        <h2 className="max-w-lg mb-6  text-4xl font-bold leading-none tracking-tight text-gray-900 pb-8 mx-auto sm:text-4xl">
          How It Works
        </h2>
      </div>

      {/* ---------- Block 1 ---------- */}
      <div class="flex justify-between flex-col gap-8 mb-20 sm:flex-row">
        <div>
          <div className="sm:text-center">
            <div className="flex items-center justify-center w-10 h-10 mb-4 text-2xl font-extrabold rounded-full text-white bg-FM-orange mx-auto">
              1
            </div>
            {/* <h6 className="mb-2 font-semibold leading-5">Slugging catcher</h6> */}
            <p className="max-w-md text-center mb-3 text-2xl text-gray-900 mx-auto sm:max-w-xs">
              Agent reaches out to Freemodel, shares basic information about the
              home. A local Project Director is selected and assigned.
            </p>
          </div>
        </div>
        <div class="order-last sm:order-none">
          <div className="sm:text-center sm:mt-32">
            <div>
              <img
                src="https://images.unsplash.com/photo-1682957376808-dcb27d61f95e?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHx0b3BpYy1mZWVkfDV8TThqVmJMYlRSd3N8fGVufDB8fHx8&auto=format&fit=crop&w=800&q=60"
                className="object-cover w-96 h-64 rounded shadow-lg sm:h-80"
              />
            </div>
          </div>
        </div>
        <div>
          <div className="sm:text-center sm:mt-60">
            <div className="flex items-center justify-center w-10 h-10 mb-4 text-2xl font-extrabold rounded-full text-white bg-FM-orange mx-auto">
              2
            </div>
            {/* <h6 className="mb-2 font-semibold leading-5">Slugging catcher</h6> */}
            <p className="max-w-md text-center mb-3 text-2xl text-gray-900 mx-auto sm:max-w-xs">
              Agent reaches out to Freemodel, shares basic information about the
              home. A local Project Director is selected and assigned.
            </p>
          </div>
        </div>
      </div>
      {/* ---------- Block 2 ---------- */}
      <div class="flex justify-between flex-col gap-8 mb-20 sm:flex-row">
        <div>
          <div className="sm:text-center">
            <div className="flex items-center justify-center w-10 h-10 mb-4 text-2xl font-extrabold rounded-full text-white bg-FM-orange mx-auto">
              3
            </div>
            {/* <h6 className="mb-2 font-semibold leading-5">Slugging catcher</h6> */}
            <p className="max-w-md text-center mb-3 text-2xl text-gray-900 mx-auto sm:max-w-xs">
              Agent reaches out to Freemodel, shares basic information about the
              home. A local Project Director is selected and assigned.
            </p>
          </div>
        </div>
        <div class="order-last sm:order-none">
          <div className="sm:text-center sm:mt-32">
            <div>
              <img
                src="https://images.unsplash.com/photo-1682957376808-dcb27d61f95e?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHx0b3BpYy1mZWVkfDV8TThqVmJMYlRSd3N8fGVufDB8fHx8&auto=format&fit=crop&w=800&q=60"
                className="object-cover w-96 h-64 rounded shadow-lg sm:h-80"
              />
            </div>
          </div>
        </div>
        <div>
          <div className="sm:text-center sm:mt-60">
            <div className="flex items-center justify-center w-10 h-10 mb-4 text-2xl font-extrabold rounded-full text-white bg-FM-orange mx-auto">
              4
            </div>
            {/* <h6 className="mb-2 font-semibold leading-5">Slugging catcher</h6> */}
            <p className="max-w-md text-center mb-3 text-2xl text-gray-900 mx-auto sm:max-w-xs">
              Agent reaches out to Freemodel, shares basic information about the
              home. A local Project Director is selected and assigned.
            </p>
          </div>
        </div>
      </div>
      {/* ---------- Block 3 ---------- */}
      <div class="flex justify-between flex-col gap-8 mb-20 sm:flex-row">
        <div>
          <div className="sm:text-center">
            <div className="flex items-center justify-center w-10 h-10 mb-4 text-2xl font-extrabold rounded-full text-white bg-FM-orange mx-auto">
              5
            </div>
            {/* <h6 className="mb-2 font-semibold leading-5">Slugging catcher</h6> */}
            <p className="max-w-md text-center mb-3 text-2xl text-gray-900 mx-auto sm:max-w-xs">
              Agent reaches out to Freemodel, shares basic information about the
              home. A local Project Director is selected and assigned.
            </p>
          </div>
        </div>
        <div class="order-last sm:order-none">
          <div className="sm:text-center sm:mt-32">
            <div>
              <img
                src="https://images.unsplash.com/photo-1682957376808-dcb27d61f95e?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHx0b3BpYy1mZWVkfDV8TThqVmJMYlRSd3N8fGVufDB8fHx8&auto=format&fit=crop&w=800&q=60"
                className="object-cover w-96 h-64 rounded shadow-lg sm:h-80"
              />
            </div>
          </div>
        </div>
        <div>
          <div className="sm:text-center sm:mt-60">
            <div className="flex items-center justify-center w-10 h-10 mb-4 text-2xl font-extrabold rounded-full text-white bg-FM-orange mx-auto">
              6
            </div>
            {/* <h6 className="mb-2 font-semibold leading-5">Slugging catcher</h6> */}
            <p className="max-w-md text-center mb-3 text-2xl text-gray-900 mx-auto sm:max-w-xs">
              Agent reaches out to Freemodel, shares basic information about the
              home. A local Project Director is selected and assigned.
            </p>
          </div>
        </div>
      </div>
      {/* ---------- Block 4 ---------- */}
      <div class="flex justify-between flex-col gap-8 mb-20 sm:flex-row">
        <div>
          <div className="sm:text-center">
            <div className="flex items-center justify-center w-10 h-10 mb-4 text-2xl font-extrabold rounded-full text-white bg-FM-orange mx-auto">
              7
            </div>
            {/* <h6 className="mb-2 font-semibold leading-5">Slugging catcher</h6> */}
            <p className="max-w-md text-center mb-3 text-2xl text-gray-900 mx-auto sm:max-w-xs">
              Agent reaches out to Freemodel, shares basic information about the
              home. A local Project Director is selected and assigned.
            </p>
          </div>
        </div>
        <div class="order-last sm:order-none">
          <div className="sm:text-center sm:mt-32">
            <div>
              <img
                src="https://images.unsplash.com/photo-1682957376808-dcb27d61f95e?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHx0b3BpYy1mZWVkfDV8TThqVmJMYlRSd3N8fGVufDB8fHx8&auto=format&fit=crop&w=800&q=60"
                className="object-cover w-96 h-64 rounded shadow-lg sm:h-80"
              />
            </div>
          </div>
        </div>
        <div>
          <div className="sm:text-center sm:mt-60">
            <div className="flex items-center justify-center w-10 h-10 mb-4 text-2xl font-extrabold rounded-full text-white bg-FM-orange mx-auto">
              8
            </div>
            {/* <h6 className="mb-2 font-semibold leading-5">Slugging catcher</h6> */}
            <p className="max-w-md text-center mb-3 text-2xl text-gray-900 mx-auto sm:max-w-xs">
              Agent reaches out to Freemodel, shares basic information about the
              home. A local Project Director is selected and assigned.
            </p>
          </div>
        </div>
      </div>
      {/* ---------- Block END ---------- */}
      <div
        className="
        flex flex-col items-center justify-center w-full mt-32"
      >
        <Ctabutton text="Let's Talk" />
      </div>
    </div>
  );
}
